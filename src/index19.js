var app = new Vue({
  ...
  methods: {
    ...
    getWeather: async function (lat, lon) {
      ...
      if (
        ...
        const localData = await this.getWeatherData(
          "https://api.met.no/weatherapi/locationforecast/1.9/.json?lat=" +
            lat +
            "&lon=" +
            lon +
            ""
    ...
    setWeather: function (weatherData) {
      ...
      this.wind = this.roundWindSpeed(
        weatherData.time[0].location.windSpeed.mps
      );
      this.rainMin = Number(
        weatherData.time[1].location.precipitation.minvalue
      );
      this.rainMax = Number(
        weatherData.time[1].location.precipitation.maxvalue
      );

      this.weatherType = weatherData.time[1].location.symbol.id;
      this.weatherImg = this.getWeatherImage(
        weatherData.time[1].location.symbol.number
      );
    ...
    getWeatherImage: function (weatherId, time) {
      if (time >= 6) {
        return (
          "https://api.met.no/weatherapi/weathericon/1.1/?content_type=image%2Fpng&symbol=" +
          weatherId
        );
      } else {
        return (
    ...
    setForecastNextThreeHours: function (forecast) {
      ...
      for (var i = 4; i < 10; i++) {
        if (i % 2 == 0) {
          ...
          var rain = this.roundRain(
            this.getAverageRain(
              forecast.time[i].location.precipitation.minvalue,
              forecast.time[i].location.precipitation.maxvalue
            )
          );
          nextThreeHoursOdd.push({
            weather: this.getWeatherImage(
              forecast.time[i].location.symbol.number,
              this.cleanTime(forecast.time[i].to)
            ),
            rain: rain,
          });
        }
    ...
    setForecastNextDays: function (forecast, indices) {
      ...
      for (var index = 0; index < forecast.time.length; index++) {
        var fulldate = String(forecast.time[indices[index]].from).substring(
          5,
          10
        );
        ...
        this.forecastNextDays.push({
          ...
          weather: this.getWeatherImage(
            forecast.time[indices[index]].location.symbol.number,
            this.cleanTime(forecast.time[indices[index]].from)
          ),
          wind: this.roundWindSpeed(
            forecast.time[indices[index] + 1].location.windSpeed.mps
          ),
